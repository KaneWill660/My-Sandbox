...
def debug_detailed():
    ...
    two_hours_ago_vietnam = now_vietnam - timedelta(hours=2)
    
    print(f"  UTC: {two_hours_ago_utc}")
    print(f"  Local: {two_hours_ago_local}")
    print(f"  Vietnam: {two_hours_ago_vietnam}")
    print()
    
    for tz_name, start_time in [
        ("UTC", two_hours_ago_utc),
        ("Local", two_hours_ago_local),
        ("Vietnam", two_hours_ago_vietnam)
    ]:
        cursor.execute('''
            SELECT COUNT(*) FROM coin_signals 
            WHERE timestamp >= ? AND chat_id = -1002385574544
        ...
        count = cursor.fetchone()[0]
        print(f"Keo trong 2h qua ({tz_name}): {count}")
