#!/usr/bin/env python3
"""
Script test summary 48 giờ để kiểm tra tin nhắn dài
"""
...
class Test48hSummary:
    ...
    def get_coin_signals(self, chat_id, hours=48):
    ...
    async def test_48h_summary(self, chat_id):
        """Test summary 48 giờ"""
        print(f"TEST 48H SUMMARY - GROUP {chat_id}")
        print("=" * 60)
        
        # Lấy signals
        signals = self.get_coin_signals(chat_id, 48)
        
        if not signals:
            print("Khong co keo nao trong 48 gio qua!")
            return
        
        print(f"Tim thay {len(signals)} keo trong 48 gio qua")
        
        # Tạo summary message
        summary_text = "TONG HOP KEO 2 NGAY QUA\n"
        ...
        if message_length > 4000:
            ...
            for i, chunk in enumerate(chunks, 1):
                ...
                print(f"  Bat dau: {chunk[:50]}...")
                print(f"  Ket thuc: ...{chunk[-50:]}")
                print()
        else:
...
if __name__ == "__main__":
    ...
    print("TESTING 48H SUMMARY")
    print("=" * 60)
    
    # Test 48 giờ
    length, count = asyncio.run(test.test_48h_summary(chat_id))
    
    print("\n" + "=" * 60)
    print("KET QUA:")
    print(f"48h: {count} keo, {length} ky tu")
    
    if length > 4000:
        print("WARNING: Tin nhan qua dai! Bot se chia nho.")
        print("Auto delete se hoat dong voi tat ca cac chunk.")
    else:
        print("OK: Tin nhan co do dai hop le")
        print("Auto delete se hoat dong binh thuong.")
